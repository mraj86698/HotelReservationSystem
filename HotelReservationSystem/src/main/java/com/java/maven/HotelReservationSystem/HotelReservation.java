...
public class HotelReservation {
	...
	/**
	 * getTime() method, representing the number of milliseconds since January 1st
	 * 1970, 00:00 1000 is used here just for converting seconds to milliseconds.
	 * Number of seconds in a day = 24 * 60 * 60 = 86400 seconds. So after
	 * calculating the expression, the result is in milliseconds.
	 *
	 * @param startDate
	 * @param endDate
	 * @return
	...
	public Customer bestRatedHotel(String type, String start_date, String end_date) {
		...
		Optional<Hotel> cheapestHotelOpt = hotelList.stream().max((Comparator.comparingInt(Hotel::getRating)));
	...
	public Customer findCheapestHotel(String type, String start_date, String end_date) {
		...
		Optional<Hotel> cheapestHotelOpt = hotelList.stream()
				.min((Comparator.comparingInt(Hotel::getTotalBill).thenComparing(Hotel::getRating)));
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			case "1": {
				...
				obj.addHotel(hotelName, rateWeekdayRegular, rateWeekendReward, rateWeekdayRegular, rateWeekendReward,
						rating);
				break;
			}
			case "2": {
